...
export default function ProcessingModal({ job, onComplete, onConfirm, onCancel }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="analysis-title">
      ...
      <div className="relative z-10 bg-zinc-900 border border-zinc-800 rounded-2xl p-6 w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <h2 id="analysis-title" className="text-lg font-semibold text-zinc-50">{done ? "Your results are ready" : failed ? "This recording cannot continue" : waiting ? "A quick quality decision" : "Preparing a reliable analysis…"}</h2>
