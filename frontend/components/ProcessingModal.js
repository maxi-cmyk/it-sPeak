  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="analysis-title">
      ...
      <div className="modal-panel max-w-lg">
        ...
        {replacing && <p className="mt-2 text-sm leading-6 text-zinc-400">This project already has five sessions. Session 1 is used as the baseline for your progress, so choose another session to replace. The selected session is only replaced after the new analysis succeeds.</p>}
        ...
        <div className="flex gap-3">
          {done ? <button onClick={onComplete} className="btn-primary w-full">View results <span aria-hidden="true">→</span></button> : waiting ? <><button onClick={onCancel} className="btn-secondary flex-1">Re-record</button><button onClick={onConfirm} className="inline-flex min-h-10 flex-1 items-center justify-center rounded-lg bg-amber-500 px-4 py-2.5 text-sm font-semibold text-zinc-50 transition-colors hover:bg-amber-400">Continue anyway</button></> : failed ? <button onClick={onCancel} className="inline-flex min-h-10 w-full items-center justify-center rounded-lg border border-red-500/40 px-4 py-2.5 text-sm font-medium text-red-700 transition-colors hover:bg-red-500/10">Close and re-record</button> : <button onClick={onCancel} className="btn-secondary w-full">Cancel analysis</button>}
        </div>
        </>}
