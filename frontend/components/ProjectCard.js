    ...
    >
      ...
      <div className="mb-4 flex flex-wrap gap-1.5">
        {(project.improvementAreas || []).map((area) => (
          <span key={area} className="text-accent rounded-full border border-blue-600/40 bg-blue-500/10 px-2.5 py-1 text-[10px] font-semibold">
            {improvementAreaLabels[area] || area}
          </span>
        ))}
      </div>

      <div className="mt-auto flex items-center gap-1.5 border-t border-zinc-800 pt-3 text-xs">
        ...
        >
          {[
            ...
            <button
              ...
              className={`flex w-full items-center px-3 py-2.5 text-left text-sm transition-colors hover:bg-zinc-700 ${danger ? "text-red-700" : "text-zinc-200"}`}
            >
              {label}
            </button>
          ))}
