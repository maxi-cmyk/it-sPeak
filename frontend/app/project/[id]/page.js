...
export default function ProjectPage() {
  ...
  const handleSessionUpload = (file) => {
    setModal("processing");
    analysisJob.start({
      file,
      projectId: id,
      archetype: "corporate_board",
      audienceContext: project?.description || "",
    });
  };
  ...
  return (
    <div className="min-h-screen bg-zinc-950">
      ...
      {modal === "addSession" && (
        <AddSessionModal
          onClose={() => setModal(null)}
          onConfirm={handleSessionUpload}
        />
      )}
      {modal === "processing" && (
        <ProcessingModal
          job={analysisJob}
          onComplete={() => router.push(`/session/${analysisJob.sessionId}?projectId=${id}`)}
          onConfirm={analysisJob.confirm}
          onCancel={() => { analysisJob.reset(); setModal(null); }}
        />
      )}
    </div>
  );
