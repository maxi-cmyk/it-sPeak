...
export default function ProjectPage() {
  ...
  return <div className="app-shell"><Navbar backHref="/" /><main className="page-container"><div className="grid grid-cols-1 gap-8 lg:grid-cols-3"><div className="flex flex-col gap-6 lg:col-span-2"><header><div className="flex flex-col items-start justify-between gap-4 sm:flex-row"><div className="min-w-0"><p className="page-kicker">{sessions.length}/5 sessions retained</p><h1 className="page-title break-words">{project.name}</h1><p className="page-summary">{project.description || "No rehearsal goal added yet."}</p></div><button onClick={openProjectEditor} className="btn-secondary w-full shrink-0 sm:w-auto" aria-label={`Edit ${project.name}`}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M12 20h9" /><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z" /></svg>Edit project</button></div><div className="mt-5 flex flex-col gap-4"><span className="chip w-fit">{archetypeLabels[project.archetype] || project.archetype}</span>{improvementAreaGroups.map((group) => { const selected = project.improvementAreas.filter((area) => improvementAreaGroupByValue[area] === group.key); if (selected.length === 0) return null; return <div key={group.key}><p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-zinc-600">{group.label} focus</p><div className="flex flex-wrap gap-2">{selected.map((area) => <span key={area} className="chip chip-selected">{improvementAreaLabels[area]}</span>)}</div></div>; })}</div></header>{project.deadline && <div className="chip w-fit"><span className={days !== null && days <= 30 ? "text-readiness" : "text-zinc-400"}>{days > 0 ? `${days} days until deadline` : "Deadline passed"}</span><span className="mx-2 text-zinc-600">·</span><span>{formatDate(project.deadline)}</span></div>}<section><div className="mb-3 flex items-center justify-between"><h2 className="text-base font-semibold text-zinc-200">Retained sessions</h2>{sessions.length > 0 && <button onClick={() => setModal("allSessions")} className="btn-quiet">View all</button>}</div>{sessions.length === 0 ? <div className="rounded-xl border border-dashed border-zinc-800 bg-zinc-900 p-8 text-center"><p className="mb-4 text-sm leading-6 text-zinc-500">Your first successful analysis becomes the protected baseline.</p><button onClick={() => setModal("addSession")} className="btn-primary">Add first session</button></div> : <div className="flex flex-col gap-3">{sessions.slice(0,3).map((session,index) => <SessionCard key={session.id} session={session} prev={sessions[index+1]} onClick={() => router.push(`/session/${session.id}`)} />)}</div>}</section></div>{latest && <aside className="flex flex-col gap-5"><div className="surface-card"><h3 className="section-label mb-4">Latest rehearsal</h3><div className="mb-4 flex justify-center"><ScoreRing score={latest.overallScore} /></div><div className="flex flex-col gap-3"><RatingBar label="Voice" value={latest.tone} target={85} /><RatingBar label="Body" value={latest.body} target={85} /><RatingBar label="Facial expressions" value={latest.face} target={85} /></div></div><button onClick={() => setModal("addSession")} className="btn-primary w-full">New session</button></aside>}</div>{sessions.length > 0 && <section className="surface-card mt-8"><h2 className="section-label mb-4">Pillar progress</h2><TimelineChart data={pillarData} /></section>}</main>
