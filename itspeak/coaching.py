...
Two providers are supported behind one interface: Google (``google-genai``) and
Anthropic (``anthropic``). If neither SDK/key is available the service degrades
to a deterministic rule-based fallback so the pipeline never hard-crashes.
"""
...
class CoachingService:
    ...
    ----------
    provider:
        Override the configured provider ("google" or "anthropic").
    """

    def __init__(self, provider: str | None = None) -> None:
        self._settings = get_settings()
        self.provider = (provider or self._settings.llm_provider).lower()
    ...
    def _call_google(self, system_prompt: str, user_prompt: str) -> str:
        ...
        if not self._settings.google_api_key:
            raise RuntimeError("ITSPEAK_GOOGLE_API_KEY is not set.")
    ...
    def _call_anthropic(self, system_prompt: str, user_prompt: str) -> str:
        ...
        if not self._settings.anthropic_api_key:
            raise RuntimeError("ITSPEAK_ANTHROPIC_API_KEY is not set.")
    ...
    def _parse_and_validate(self, raw: str) -> list[CoachingCard]:
        ...
        per_module: dict[Module, int] = {Module.FACE: 0, Module.BODY: 0}
